-- Adiciona columnId ao extraColumns do get_my_work_items
-- Necessario para edição inline de colunas no MyWork/TeamWork
...
BEGIN
  ...
  item_status AS (
    SELECT DISTINCT ON (cv.item_id)
      cv.item_id,
      c.id AS status_column_id,
      c.settings AS status_settings,
  ...
  item_people AS (
    SELECT cv.item_id,
           MIN(cv.column_id::text) AS people_column_id,
      ...
      jsonb_build_object(
        ...
        'statusValue', CASE
          WHEN ist.status_key IS NOT NULL THEN jsonb_build_object(
            ...
            'columnId', ist.status_column_id,
            'settings', ist.status_settings
          )
          ELSE NULL
        END,
        'dateValue',   id2.date_str,
        'dateColumnId', id2.date_column_id,
        'startTime',   id2.start_time,
        'endTime',     id2.end_time,
        'peopleColumnId', ip.people_column_id,
